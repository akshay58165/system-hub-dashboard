...
import { Activity, Apple, BatteryCharging, Brain, CircleMinus, CirclePlus, Droplets, Dumbbell, Eye, Heart, Home, Lightbulb, LockKeyhole, Moon, ScanLine, Smile, Sparkles, WalletCards, X } from 'lucide-react';
...
function stateFor(value: number) {
  if (value === 0) return { label: 'Not recorded', text: 'text-zinc-600', bg: 'bg-zinc-800', border: 'border-zinc-900' };
...
function SignalGraph({ node, entries }: { node: CalibrationNode; entries: WellbeingEntry[] }) {
  ...
  const currentY = node.value > 0 ? 34 - ((node.value - 1) / 9) * 27 : 34;
  return <div className="relative h-12 rounded-md border border-zinc-900 bg-black/30 overflow-hidden">
    <svg viewBox="0 0 100 38" preserveAspectRatio="none" className="h-full w-full">
      ...
      {points.length > 1 ? <polyline points={points.join(' ')} fill="none" stroke={node.color} strokeWidth="1.8" vectorEffect="non-scaling-stroke" /> : node.value > 0 ? <line x1="2" y1={currentY} x2="98" y2={currentY} stroke={node.color} strokeWidth="1" strokeDasharray="3 3" opacity="0.45" /> : null}
      {node.value > 0 && <circle cx={points.length ? Number(points.at(-1)?.split(',')[0]) : 98} cy={points.length ? Number(points.at(-1)?.split(',')[1]) : currentY} r="1.6" fill={node.color} />}
    </svg>
...
export default function DailyStateDashboard({ nodes, history, onUpdateNode }: Props) {
  ...
  const [showDailyPrompt, setShowDailyPrompt] = useState(() => todayHistory.length === 0);
  const [draft, setDraft] = useState<Record<string, string>>(() => Object.fromEntries(nodes.map(node => [node.id, ''])));
  useEffect(() => {
    if (todayHistory.length === 0) {
      setDraft(Object.fromEntries(nodes.map(node => [node.id, ''])));
      setShowDailyPrompt(true);
    }
  }, [todayHistory.length]);
  const saveDailyCheckIn = (event: React.FormEvent) => {
    event.preventDefault();
    if (!nodes.every(node => Number(draft[node.id]) >= 1)) return;
    nodes.forEach(node => onUpdateNode(node.id, Number(draft[node.id]), true));
    setShowDailyPrompt(false);
  };
  const sleepRecorded = todayHistory.some(entry => entry.nodeId === 'sleep');
  const averageFor = (group: 'BODY' | 'MIND' | 'CONTEXT') => {
    const values = nodes.filter(node => META[node.id]?.group === group && node.value > 0).map(node => node.value);
    return values.length ? Math.round((values.reduce((sum, value) => sum + value, 0) / values.length) * 10) : null;
  };
  const sorted = nodes.filter(node => node.value > 0).sort((a, b) => a.value - b.value);
  ...
  return <div id="wellbeing-dashboard" className="space-y-4 scroll-mt-4">
    {showDailyPrompt && <div className="fixed inset-0 z-50 grid place-items-center bg-black/80 p-4 backdrop-blur-sm">
      <form onSubmit={saveDailyCheckIn} className="max-h-[90vh] w-full max-w-3xl overflow-y-auto rounded-xl border border-cyan-900/60 bg-zinc-950 p-5 shadow-[0_0_60px_rgba(34,211,238,0.12)]">
        <div className="flex items-start justify-between gap-4 border-b border-zinc-900 pb-4"><div><span className="text-[9px] font-mono font-bold tracking-[0.18em] text-cyan-400">NEW DAY · BLANK STATE</span><h2 className="mt-1 text-lg font-bold text-white">Record today’s starting parameters</h2><p className="mt-1 text-[10px] text-zinc-500">Nothing is pre-filled. Every field must come from you, and only today’s entries will affect today’s metrics.</p></div><button type="button" onClick={() => setShowDailyPrompt(false)} className="rounded border border-zinc-800 p-2 text-zinc-600 hover:text-white" title="Fill directly on the panel"><X className="h-4 w-4" /></button></div>
        <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">{nodes.map(node => { const meta = META[node.id]; const Icon = meta?.icon || Activity; return <label key={node.id} className="rounded-lg border border-zinc-900 bg-zinc-900/20 p-3"><span className="flex items-center gap-2 text-[9px] font-mono font-bold text-zinc-300"><Icon className="h-3.5 w-3.5" style={{ color: node.color }} />{node.label}</span><span className="mt-1 block min-h-5 text-[7px] text-zinc-600">{meta?.hint}</span><select required value={draft[node.id] || ''} onChange={event => setDraft(current => ({ ...current, [node.id]: event.target.value }))} className="mt-2 w-full rounded border border-zinc-800 bg-zinc-950 px-3 py-2 text-[10px] font-mono text-zinc-300 outline-none focus:border-cyan-700"><option value="" disabled>SELECT 1–10</option>{Array.from({ length: 10 }, (_, index) => index + 1).map(value => <option key={value} value={value}>{value} / 10</option>)}</select></label>; })}</div>
        <div className="mt-5 flex flex-col-reverse gap-2 border-t border-zinc-900 pt-4 sm:flex-row sm:justify-end"><button type="button" onClick={() => setShowDailyPrompt(false)} className="rounded border border-zinc-800 px-4 py-2.5 text-[9px] font-mono font-bold text-zinc-500 hover:text-white">FILL ON PANEL</button><button disabled={!nodes.every(node => Number(draft[node.id]) >= 1)} className="rounded bg-cyan-400 px-5 py-2.5 text-[9px] font-mono font-black text-zinc-950 disabled:cursor-not-allowed disabled:opacity-30">SAVE TODAY’S STARTING STATE</button></div>
      </form>
    </div>}
    <header className="bg-zinc-950 border border-zinc-900 rounded-xl p-4 grid grid-cols-1 lg:grid-cols-[1fr_auto] gap-4 items-center relative overflow-hidden">
      ...
      <div className="flex gap-2 font-mono"><div className="min-w-28 rounded-lg border border-zinc-800 bg-zinc-900/40 p-3 text-center"><span className="text-[7px] text-zinc-500 block">READINESS NOW</span><strong className={`text-xl ${readiness ? stateFor(Math.round(readiness / 10)).text : 'text-zinc-600'}`}>{readiness ? `${readiness}%` : 'N/A'}</strong></div><div className="min-w-28 rounded-lg border border-zinc-800 bg-zinc-900/40 p-3 text-center"><span className="text-[7px] text-zinc-500 block">UPDATED TODAY</span><strong className="text-xl text-cyan-400">{latestByNode.length}<span className="text-[9px] text-zinc-600">/{nodes.length}</span></strong></div></div>
    </header>
    ...
    <div className="grid grid-cols-1 xl:grid-cols-[minmax(0,2fr)_minmax(340px,0.8fr)] gap-4 items-start">
      <section className="bg-zinc-950 border border-zinc-900 rounded-xl p-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 2xl:grid-cols-3 gap-3">{nodes.map(node => {
          ...
          return <article key={node.id} className={`rounded-lg border ${state.border} bg-zinc-900/20 p-3 shadow-[inset_0_1px_0_rgba(255,255,255,0.02)] hover:bg-zinc-900/35 transition-colors`}>
            <div className="flex items-start justify-between gap-3"><div className="flex gap-2.5"><div className={`h-9 w-9 rounded-md border ${state.border} bg-black/30 grid place-items-center shadow-inner`}><Icon className={`h-4 w-4 ${state.text}`} /></div><div><div className="flex items-center gap-2"><h4 className="text-[10px] font-mono font-bold text-zinc-200">{node.label}</h4><span className={`text-[7px] font-mono ${state.text}`}>{state.label.toUpperCase()}</span></div><p className="text-[7px] text-zinc-600 mt-0.5">{meta?.hint}</p></div></div><div className="text-right"><strong className={`text-xl font-mono ${state.text}`}>{node.value || '—'}</strong>{node.value > 0 && <span className="text-[8px] text-zinc-700">/10</span>}</div></div>
            <div className="grid grid-cols-[28px_1fr_28px] gap-2 items-center mt-3"><button disabled={locked} onClick={() => change(node.value > 1 ? node.value - 1 : 1)} className="h-7 rounded border border-zinc-800 bg-zinc-950 text-zinc-500 hover:text-white hover:border-zinc-600 disabled:opacity-30 grid place-items-center"><CircleMinus className="h-3.5 w-3.5" /></button><div><input aria-label={`${node.label} rating`} type="range" min="1" max="10" step="1" value={node.value || 1} disabled={locked} onChange={event => onUpdateNode(node.id, Number(event.target.value), false)} onPointerUp={event => onUpdateNode(node.id, Number(event.currentTarget.value), true)} onKeyUp={event => onUpdateNode(node.id, Number(event.currentTarget.value), true)} className={`w-full h-2 accent-cyan-400 cursor-pointer disabled:opacity-30 ${node.value === 0 ? 'opacity-35' : ''}`} /><div className="flex justify-between px-0.5 mt-0.5">{Array.from({ length: 10 }).map((_, index) => <i key={index} className={`h-1 w-px ${node.value > 0 && index < node.value ? state.bg : 'bg-zinc-800'}`} />)}</div></div><button disabled={locked} onClick={() => change(node.value > 0 ? node.value + 1 : 1)} className="h-7 rounded border border-zinc-800 bg-zinc-950 text-zinc-500 hover:text-white hover:border-zinc-600 disabled:opacity-30 grid place-items-center"><CirclePlus className="h-3.5 w-3.5" /></button></div>
      ...
      <aside className="space-y-4 xl:sticky xl:top-4">
        ...
        <section className="bg-zinc-950 border border-zinc-900 rounded-xl p-4"><div className="flex items-center gap-2 border-b border-zinc-900 pb-3 mb-3"><Brain className="h-4 w-4 text-purple-400" /><h3 className="text-xs font-mono font-bold text-white">STATE BALANCE</h3></div><div className="space-y-3">{(['BODY','MIND','CONTEXT'] as const).map(group => { const value = averageFor(group); const state = stateFor(value === null ? 0 : Math.round(value / 10)); return <div key={group}><div className="flex justify-between text-[8px] font-mono mb-1"><span className="text-zinc-500">{group}</span><strong className={state.text}>{value === null ? 'N/A · NOT RECORDED' : `${value}% · ${state.label}`}</strong></div><div className="h-2 rounded-full bg-zinc-900 overflow-hidden"><div className={`h-full rounded-full ${state.bg}`} style={{ width: `${value || 0}%` }} /></div></div>; })}</div><div className="grid grid-cols-2 gap-2 mt-4"><div className="rounded border border-rose-900/40 bg-rose-950/10 p-2"><span className="text-[7px] text-zinc-600 block">LOWEST NOW</span><strong className="text-[9px] text-rose-400">{sorted[0] ? `${sorted[0].label} ${sorted[0].value}/10` : 'N/A'}</strong></div><div className="rounded border border-emerald-900/40 bg-emerald-950/10 p-2"><span className="text-[7px] text-zinc-600 block">STRONGEST NOW</span><strong className="text-[9px] text-emerald-400">{sorted.at(-1) ? `${sorted.at(-1)?.label} ${sorted.at(-1)?.value}/10` : 'N/A'}</strong></div></div></section>
