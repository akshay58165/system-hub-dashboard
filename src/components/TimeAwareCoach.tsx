...
export default function TimeAwareCoach({ goals, nodes, wellbeingHistory, onNavigate }: Props) {
  ...
  const notices = useMemo<CoachNotice[]>(() => {
    ...
    if (hour >= 11 && hour < 14) items.push({ id: 'lunch', label: 'MEAL WINDOW', message: 'Pause for lunch instead of working through it.', reason: value('nutrition') > 0 && value('nutrition') <= 6 ? 'Your recorded nutrition score supports a proper meal now.' : 'A planned meal protects afternoon focus.', tone: 'text-amber-400 border-amber-900/50', icon: <Sandwich className="h-4 w-4" /> });
    if (hour >= 20 || hour < 2) items.push({ id: 'sleep', label: 'RECOVERY WINDOW', message: value('sleep') > 0 && value('sleep') <= 5 ? 'Start winding down now and aim for an early night.' : 'Protect tomorrow by closing the workday deliberately.', reason: value('sleep') > 0 && value('sleep') <= 5 ? `Last recorded sleep is ${value('sleep')}/10.` : 'Late work can erode tomorrow’s available energy.', tone: 'text-indigo-300 border-indigo-900/50', icon: <Moon className="h-4 w-4" /> });
    if ((value('hydration') > 0 && value('hydration') <= 6) || (hour >= 9 && hour <= 21)) items.push({ id: 'water', label: 'HYDRATION CHECK', message: 'Drink water now, then update hydration if it changes.', reason: value('hydration') > 0 && value('hydration') <= 6 ? `Hydration is currently ${value('hydration')}/10.` : 'A small scheduled check prevents a late-day deficit.', tone: 'text-cyan-400 border-cyan-900/50', icon: <Droplets className="h-4 w-4" /> });
    const [startHour] = goals.workWindowStart.split(':').map(Number); const [endHour] = goals.workWindowEnd.split(':').map(Number);
    if (hour >= startHour && hour <= endHour) items.push({ id: 'break', label: 'WORKLOAD PACER', message: 'Take a five-minute eye and movement break before the next deep block.', reason: (value('eyeComfort') > 0 && value('eyeComfort') <= 6) || (value('physicalComfort') > 0 && value('physicalComfort') <= 6) ? 'Your recorded comfort signals show that recovery is useful.' : 'Short breaks protect accuracy across a long work window.', tone: 'text-emerald-400 border-emerald-900/50', icon: <StretchHorizontal className="h-4 w-4" /> });
    if (value('energy') > 0 && value('energy') <= 4) items.unshift({ id: 'energy', label: 'LOW ENERGY MODE', message: 'Choose one short, concrete task; avoid opening a new deep-work thread.', reason: `Recorded energy is ${value('energy')}/10.`, tone: 'text-rose-400 border-rose-900/50', icon: <BellRing className="h-4 w-4" /> });
    return items;
