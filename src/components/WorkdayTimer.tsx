  ...
  const renderTopicPicker = () => (
    <div className="relative mt-1">
      ...
      <AnimatePresence>{topicPickerOpen && <motion.div initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -4 }} className="absolute inset-x-0 top-full z-[120] mt-1.5 max-h-72 space-y-1 overflow-y-auto rounded-xl border border-neutral-800 bg-neutral-950 p-1.5 shadow-[0_20px_60px_rgba(0,0,0,.75)] [scrollbar-color:#52525b_#09090b] [scrollbar-width:thin] [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-neutral-700 [&::-webkit-scrollbar-track]:bg-neutral-950 [&::-webkit-scrollbar]:w-1.5">
        {rankedTopics.map((topic, index) => { const guidance = topicGuidance(topic); const remainingStages = goalStages.filter(stage => stageOrder.indexOf(stage) > stageOrder.indexOf(topic.status)); return <div key={topic.id} className="group rounded-lg border border-transparent bg-neutral-900/45 p-2.5 transition hover:border-purple-800/60 hover:bg-purple-950/20"><button type="button" onClick={() => { chooseTopic(topic.id); setTopicPickerOpen(false); }} className="w-full text-left">
        ...
        </button><div className="ml-8 mt-2 border-t border-neutral-800/70 pt-2"><span className="mb-1 block text-[7px] uppercase tracking-wider text-neutral-600">Choose a remaining finish stage</span><div className="flex flex-wrap gap-1">{remainingStages.map(stage => <button key={stage} type="button" onClick={() => { chooseTopic(topic.id); setGoalTarget(stage); setTopicPickerOpen(false); }} className="rounded border border-purple-900/50 bg-purple-950/20 px-1.5 py-1 text-[7px] font-bold capitalize text-purple-300 transition hover:border-purple-500 hover:bg-purple-500/20 hover:text-white">{stage}</button>)}</div></div></div>; })}
        ...
        {showSetup && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/75 p-4 backdrop-blur-sm">
            <motion.div ref={setupCardRef} initial={{ opacity: 0, scale: .96 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: .96 }} className="w-full max-w-md rounded-2xl border border-cyan-900/50 bg-neutral-950 p-5 shadow-[0_0_50px_rgba(6,182,212,.12)]">
              <div className="flex items-center justify-between"><div><h2 className="text-base font-bold text-white">{setupStep === 'budget' ? 'Set work budget' : 'Set today\'s goals'}</h2><p className="mt-1 text-[10px] text-neutral-500">{setupStep === 'budget' ? 'Choose today\'s active work quota. The timer will not start yet.' : 'Optional. Add topic milestones, or begin without goals.'}</p></div><button onClick={() => setShowSetup(false)} className="p-1 text-neutral-500 hover:text-white"><X className="h-4 w-4" /></button></div>
              {setupStep === 'budget' ? <>
                ...
                <button onClick={() => setSetupStep('goals')} className="mt-5 w-full rounded-lg bg-cyan-500 py-2.5 text-xs font-bold text-black hover:bg-cyan-400">Continue to goals</button>
              </> : <>
                ...
                <div className="mt-5 grid grid-cols-2 gap-2"><button onClick={() => startDay([])} className="rounded-lg border border-neutral-700 bg-neutral-900 py-2.5 text-[10px] font-bold text-neutral-200 hover:border-neutral-500">Start without goals</button><button disabled={!draftGoals.length} onClick={() => startDay(draftGoals)} className="rounded-lg bg-cyan-500 py-2.5 text-[10px] font-bold text-black hover:bg-cyan-400 disabled:cursor-not-allowed disabled:opacity-35">Start with {draftGoals.length || ''} goal{draftGoals.length === 1 ? '' : 's'}</button></div>
                <button onClick={() => setSetupStep('budget')} className="mt-3 w-full text-[9px] text-neutral-500 hover:text-white">Back to work budget</button>
              </>}
            </motion.div>
          </div>
        )}
