        ...
        {session && showProductivityPrompt && (
          <div className="fixed inset-0 z-[10050] flex items-center justify-center bg-black/80 p-4 backdrop-blur-sm">
            <motion.div initial={{ opacity: 0, scale: .96 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: .96 }} className="w-full max-w-md rounded-2xl border border-purple-800/60 bg-neutral-950 p-5 shadow-[0_0_50px_rgba(168,85,247,.16)]">
              ...
              <p className="mt-1 text-[10px] text-neutral-500">{productivityPromptMode === 'stop' ? 'Score the work since your last break, then end the session.' : 'Choose once to record this work segment and pause the timer.'}</p>
              ...
              <button type="button" onClick={() => { setShowProductivityPrompt(false); setProductivityPromptMode('pause'); }} className="mt-4 w-full text-center text-[9px] text-neutral-500 hover:text-white">{productivityPromptMode === 'stop' ? 'Cancel — keep working' : 'Keep timer running'}</button>
            </motion.div>
          </div>
        )}
            ...
            <motion.div className="fixed inset-0 z-[1000] flex items-start justify-center overflow-y-auto bg-black/80 px-4 py-5 backdrop-blur-sm sm:items-center" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setShowGoals(false)}>
              <motion.div initial={{ opacity: 0, scale: .96 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: .96 }} className="max-h-[calc(100dvh-2.5rem)] w-full max-w-xl overflow-y-auto rounded-2xl border border-purple-900/50 bg-neutral-950 p-5 shadow-[0_0_50px_rgba(168,85,247,.12)] sm:p-6" onClick={e => e.stopPropagation()}>
                ...
                <div className="mt-5 space-y-5 rounded-2xl border border-neutral-800 bg-neutral-900/30 p-5">
                  ...
                  {lastGoalAdded && <div className="text-[9px] text-emerald-300"><Check className="mr-1 inline h-3 w-3" />{lastGoalAdded} added. You can add another.</div>}
                  <div className="block text-xs font-bold uppercase tracking-wider text-neutral-400">Topic{renderTopicPicker()}</div>
                  {selectedTopic && <div className="rounded-lg bg-neutral-950 p-2.5"><div className="truncate text-[10px] font-semibold text-white">{selectedTopic.name}</div><div className="mt-1 text-[8px] uppercase text-cyan-300">Current {stageLabel[selectedTopic.status] || selectedTopic.status} · Priority {selectedTopic.priority}</div></div>}
                  <div className="block text-xs font-bold uppercase tracking-wider text-neutral-400">Milestone to reach today{renderMilestonePicker()}</div>
                  ...
                  {editingGoalId && <button onClick={() => { setEditingGoalId(null); setGoalTopicId(''); }} className="w-full text-center text-[9px] text-neutral-500 hover:text-white">Cancel editing</button>}
                  {!rankedTopics.length && <div className="text-center text-[8px] text-neutral-600">No unfinished topics available.</div>}
                </div>
