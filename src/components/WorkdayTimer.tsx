...
function GoalTrail({
  ...
  return (
    <div className="mt-1.5 flex items-center gap-2">
      ...
      <span className={`shrink-0 font-mono text-[13px] uppercase tracking-wider ${palette.includes('text-emerald') ? 'text-emerald-200' : palette.includes('text-amber') ? 'text-amber-200' : palette.includes('text-purple') ? 'text-purple-200' : 'text-cyan-200'}`}>
...
export default function WorkdayTimer({ session, setSession, topics, onEndSession, onDiscardSession, onOpenTopic, onExternalPause, onExternalResume, onRemoveGoal }: WorkdayTimerProps) {
  ...
  // When stopping while actively running, the productivity prompt scores the
  // final (last resume → stop) segment before the end-confirmation appears.
  const [productivityPromptMode, setProductivityPromptMode] = useState<'pause' | 'stop'>('pause');
  const [pendingFinalScore, setPendingFinalScore] = useState<number | undefined>(undefined);
  ...
  const renderTopicPicker = () => (
    <div className="relative mt-1">
      <button type="button" onClick={() => setTopicPickerOpen(open => !open)} aria-expanded={topicPickerOpen} className={`flex w-full items-center justify-between gap-3 rounded-xl border px-4 py-3 text-left transition ${topicPickerOpen ? 'border-purple-600 bg-purple-950/20 shadow-[0_0_18px_rgba(168,85,247,.12)]' : 'border-neutral-800 bg-neutral-950 hover:border-neutral-700'}`}>
        {selectedTopic ? <span className="min-w-0"><span className="block truncate text-sm font-semibold text-white">{selectedTopic.name}</span><span className="mt-1 block text-[14px] uppercase text-neutral-500">{selectedTopic.channel} · current {selectedTopic.status}</span></span> : <span className="text-sm font-semibold text-neutral-400">Select a ranked topic</span>}
      ...
      <AnimatePresence>{topicPickerOpen && <motion.div initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -4 }} className="absolute inset-x-0 top-full z-[120] mt-1.5 max-h-72 space-y-1 overflow-y-auto rounded-xl border border-neutral-800 bg-neutral-950 p-1.5 shadow-[0_20px_60px_rgba(0,0,0,.75)] [scrollbar-color:#52525b_#09090b] [scrollbar-width:thin] [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-neutral-700 [&::-webkit-scrollbar-track]:bg-neutral-950 [&::-webkit-scrollbar]:w-1.5">
        {rankedTopics.map((topic, index) => { const guidance = topicGuidance(topic); const remainingStages = goalStages.filter(stage => stageOrder.indexOf(stage) > stageOrder.indexOf(topic.status)); return <div key={topic.id} className="group rounded-lg border border-transparent bg-neutral-900/45 p-3 transition hover:border-purple-800/60 hover:bg-purple-950/20"><button type="button" onClick={() => { chooseTopic(topic.id); setTopicPickerOpen(false); }} className="w-full text-left">
          <div className="flex items-start gap-3"><span className="mt-0.5 font-mono text-[14px] text-neutral-600">{String(index + 1).padStart(2, '0')}</span><span className={`mt-1 h-2.5 w-2.5 shrink-0 rounded-full shadow-[0_0_8px_currentColor] ${guidance.dot}`} /><span className="min-w-0 flex-1"><span className="block text-sm font-bold leading-snug text-neutral-100 group-hover:text-white">{topic.name}</span><span className="mt-1.5 flex flex-wrap gap-1"><span className="rounded border border-blue-900/40 bg-blue-950/20 px-2 py-0.5 text-[13px] uppercase text-blue-300">{topic.status}</span><span className="rounded border border-emerald-900/40 bg-emerald-950/20 px-2 py-0.5 text-[13px] uppercase text-emerald-300">{topic.revenueLevel || `P${topic.priority}`}</span><span className={`rounded border px-2 py-0.5 text-[13px] font-bold uppercase ${guidance.tone}`}>{guidance.label}</span></span><span className="mt-2 block text-[14px] leading-relaxed text-neutral-500"><span className="text-purple-300">Next:</span> {guidance.action} · {guidance.detail}</span></span></div>
        </button><div className="ml-10 mt-2 border-t border-neutral-800/70 pt-2"><span className="mb-1.5 block text-[13px] uppercase tracking-wider text-neutral-600">Choose a remaining finish stage</span><div className="flex flex-wrap gap-1.5">{remainingStages.map(stage => <button key={stage} type="button" onClick={() => { chooseTopic(topic.id); setGoalTarget(stage); setTopicPickerOpen(false); }} className="rounded border border-purple-900/50 bg-purple-950/20 px-2.5 py-1.5 text-[13px] font-bold capitalize text-purple-300 transition hover:border-purple-500 hover:bg-purple-500/20 hover:text-white">{stageLabel[stage] || stage}</button>)}</div></div></div>; })}
        {!rankedTopics.length && <div className="px-3 py-5 text-center text-[13px] text-neutral-600">No unfinished topics available.</div>}
  ...
  return (
    <>
      {session ? (
        <div className={`relative flex shrink-0 items-stretch overflow-hidden rounded-lg border font-mono text-[14px] font-bold transition ${session.status === 'paused' ? 'border-amber-800/60 bg-amber-950/25 text-amber-300' : 'border-emerald-800/60 bg-emerald-950/25 text-emerald-300'}`}>
        ...
        <div className="flex shrink-0 items-stretch gap-0 overflow-hidden rounded-lg border border-amber-800/60 font-mono text-[14px] font-bold">
        ...
        <button type="button" onClick={openSetup} className="flex shrink-0 items-center gap-2 rounded-lg border border-cyan-900/60 bg-cyan-950/20 px-3 py-1.5 font-mono text-[14px] font-bold text-cyan-300 transition hover:border-cyan-700">
            ...
            <motion.div className="fixed inset-0 z-[1000] flex items-start justify-center overflow-y-auto bg-black/80 px-4 py-5 backdrop-blur-sm sm:items-center" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                ...
                {setupStep === 'budget' ? <>
                  ...
                  <div className="mt-5 space-y-5 rounded-2xl border border-neutral-800 bg-neutral-900/30 p-5">
                    ...
                    {!rankedTopics.length && <div className="text-center text-[13px] text-neutral-600">No unfinished topics available.</div>}
                  </div>
          ...
          {showPendingReview && pendingGoalsFromLastSession.length > 0 && !session && (
            <motion.div className="fixed inset-0 z-[1000] flex items-start justify-center overflow-y-auto bg-black/80 px-4 py-5 backdrop-blur-sm sm:items-center" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setShowPendingReview(false)}>
              <motion.div initial={{ opacity: 0, scale: .96 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: .96 }} className="w-full max-w-md rounded-2xl border border-amber-900/50 bg-neutral-950 p-5 shadow-[0_0_50px_rgba(245,158,11,.1)]" onClick={e => e.stopPropagation()}>
                ...
                <p className="mt-2 text-[14px] leading-relaxed text-neutral-500">These goals were not completed in your last session. You can jump to a topic to finish it, or start a new day.</p>
                    ...
                    return (
                      <button key={`${goal.topicId}-${goal.targetStatus}`} type="button" onClick={() => { if (onOpenTopic) { onOpenTopic(topic.id); setShowPendingReview(false); } }} className="group flex w-full items-start gap-3 rounded-xl border border-neutral-800 bg-neutral-900/40 p-3 text-left transition hover:border-amber-800/60 hover:bg-amber-950/10">
                        ...
                        <span className="min-w-0 flex-1">
                          <span className="block text-[14px] font-semibold text-neutral-100 group-hover:text-white">{topic.name}</span>
                          <span className="mt-1 flex flex-wrap gap-1">
                            <span className="rounded border border-cyan-900/50 bg-cyan-950/20 px-1.5 py-0.5 text-[12px] font-bold uppercase text-cyan-300">{remaining.length ? remaining.join(' → ') : goal.targetStatus}</span>
                            <span className="rounded border border-neutral-800 bg-neutral-900/50 px-1.5 py-0.5 text-[12px] font-bold uppercase text-neutral-400">{topic.channel}</span>
                          </span>
                        </span>
                      </button>
                    );
                ...
                <button type="button" onClick={() => setShowPendingReview(false)} className="mt-3 w-full rounded-xl border border-neutral-800 py-2 text-[14px] font-bold text-neutral-500 transition hover:border-neutral-600 hover:text-neutral-300" title="Keep these pending — you can reopen this list from the header any time">I&apos;ll do it later</button>
              </motion.div>
            </motion.div>
          )}
        </AnimatePresence>,
      ...
      <AnimatePresence>
        {session && showPanel && (
          <motion.div ref={panelRef} initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} className="fixed right-4 top-16 bottom-4 z-[90] w-[min(380px,calc(100vw-2rem))] overflow-y-auto overscroll-contain rounded-2xl border border-neutral-800 bg-neutral-950/98 p-4 pb-6 shadow-2xl backdrop-blur-xl">
            <div className="flex items-start justify-between"><div><div className="flex items-center gap-2 text-sm font-bold text-white"><span className={`h-2 w-2 rounded-full ${session.status === 'paused' ? 'bg-amber-400' : 'animate-pulse bg-emerald-400'}`} />Workday timer</div><div className="mt-1 text-[13px] uppercase text-neutral-600">Started {new Date(session.startedAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</div></div><button onClick={() => setShowPanel(false)} className="text-neutral-600 hover:text-white"><X className="h-4 w-4" /></button></div>
            <div className="mt-4 text-center font-mono text-3xl font-black text-white">{formatDuration(metrics.active)}</div>
            <div className="mt-1 text-center text-[13px] uppercase text-neutral-500">active work recorded</div>
            ...
            <div className="mt-3 grid grid-cols-2 gap-2 text-center sm:grid-cols-4"><div className="rounded-lg bg-neutral-900/60 p-2"><div className="text-xs font-bold text-emerald-300">{metrics.progress.toFixed(1)}%</div><div className="mt-1 text-[12px] uppercase text-neutral-600">quota filled</div></div><div className="rounded-lg bg-neutral-900/60 p-2"><div className="text-xs font-bold text-cyan-300">{formatDuration(metrics.remaining)}</div><div className="mt-1 text-[12px] uppercase text-neutral-600">remaining</div></div><div className="rounded-lg bg-neutral-900/60 p-2"><div className="text-xs font-bold text-purple-300">{metrics.productivePercent.toFixed(0)}%</div><div className="mt-1 text-[12px] uppercase text-neutral-600">productive</div></div><div className="rounded-lg bg-neutral-900/60 p-2"><div className="text-xs font-bold text-amber-300">{formatDuration(metrics.paused)}</div><div className="mt-1 text-[12px] uppercase text-neutral-600">paused</div></div></div>
            <div className="mt-4 rounded-xl border border-neutral-900 bg-neutral-900/25 p-3">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2 text-[14px] font-bold text-neutral-300">
                ...
                <div className="flex gap-2">
                  {!(session.goals || []).filter(goal => topics.some(t => t.id === goal.topicId)).length && !session.sessionNote && (
                    <button onClick={() => { setIsEditingNote(true); setNoteInput(''); }} className="flex items-center gap-1 rounded-md border border-neutral-700/50 bg-neutral-800/25 px-2 py-1 text-[13px] font-bold text-neutral-300 hover:border-neutral-500">
                      <Plus className="h-3 w-3" />Add note
                    </button>
                  )}
                  <button onClick={() => { setShowGoals(true); setGoalTopicId(''); setEditingGoalId(null); setLastGoalAdded(''); }} className="flex items-center gap-1 rounded-md border border-purple-900/50 bg-purple-950/25 px-2 py-1 text-[13px] font-bold text-purple-300 hover:border-purple-700">
              ...
              ) : <div className="mt-2 text-[13px] text-neutral-600">Optional - no topic goal set.</div>}
              {isEditingNote && (
                <div className="mt-2 flex items-center gap-2">
                  <input
                    ...
                    className="flex-1 rounded-md border border-neutral-700 bg-neutral-900 px-2 py-1 text-[14px] text-white focus:border-cyan-500 focus:outline-none"
                    autoFocus
                    onKeyDown={e => {
                      if (e.key === 'Enter') {
                  ...
                  />
                  <button onClick={() => { setSession(s => s ? { ...s, sessionNote: noteInput } : s); setIsEditingNote(false); }} className="rounded-md bg-cyan-600 px-2 py-1 text-[14px] font-bold text-white hover:bg-cyan-500">Save</button>
                  <button onClick={() => setIsEditingNote(false)} className="rounded-md border border-neutral-700 px-2 py-1 text-[14px] text-neutral-400 hover:text-white">Cancel</button>
                </div>
              )}
            </div>
            {metrics.remaining > 0 && metrics.remaining < ONE_HOUR_MS && (
              <div className="mt-3 rounded-lg border border-amber-800/50 bg-amber-950/15 p-2.5">
                <div className="text-[13px] font-bold uppercase text-amber-300">Less than 1 hour left — extend the session?</div>
                <div className="mt-2 flex gap-1.5">
                  {[10, 30, 60].map(minutes => (
                    <button key={minutes} onClick={() => extendSession(minutes)} className="flex-1 rounded-md border border-amber-700/60 bg-amber-500/10 py-1.5 text-[13px] font-bold text-amber-200 hover:bg-amber-500/20">
            ...
            <div className="mt-4 flex gap-2"><button onClick={session.status === 'paused' ? resume : pause} className={`flex flex-1 items-center justify-center gap-2 rounded-lg py-2 text-[14px] font-bold ${session.status === 'paused' ? 'bg-emerald-500 text-black' : 'bg-amber-500 text-black'}`}>{session.status === 'paused' ? <Play className="h-3.5 w-3.5" /> : <Pause className="h-3.5 w-3.5" />}{session.status === 'paused' ? 'Resume work' : 'Pause work'}</button><button onClick={endSession} title="End session" className="rounded-lg border border-rose-900/60 px-3 py-2 text-[14px] font-bold text-rose-400 hover:border-rose-700 hover:bg-rose-950/30">Stop</button></div>
          </motion.div>
        )}
        ...
        {session && showProductivityPrompt && (
          <div className="fixed inset-0 z-[10050] flex items-center justify-center bg-black/80 p-4 backdrop-blur-sm">
            <motion.div initial={{ opacity: 0, scale: .96 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: .96 }} className="w-full max-w-md rounded-2xl border border-purple-800/60 bg-neutral-950 p-5 shadow-[0_0_50px_rgba(168,85,247,.16)]">
              <h2 className="text-base font-bold text-white">{productivityPromptMode === 'stop' ? 'How productive was this final stretch?' : 'How productive was this session?'}</h2>
              <p className="mt-1 text-[14px] text-neutral-500">{productivityPromptMode === 'stop' ? 'Score the work since your last break, then end the session.' : 'Choose once to record this work segment and pause the timer.'}</p>
              <div className="mt-5 grid grid-cols-5 gap-2 sm:grid-cols-10">
                {Array.from({ length: 10 }, (_, index) => index + 1).map(score => (
                  <button key={score} type="button" onClick={() => {
                    setShowProductivityPrompt(false);
                    if (productivityPromptMode === 'stop') {
                      setPendingFinalScore(score);
                      setProductivityPromptMode('pause');
                      setShowEndConfirmation(true);
                    } else {
                      commitPause(score);
                    }
                  }} className="flex aspect-square items-center justify-center rounded-lg border border-purple-800/60 bg-purple-950/25 font-mono text-sm font-black text-purple-200 transition hover:border-purple-400 hover:bg-purple-500 hover:text-black" title={`${score * 10}% productive`}>
                    {score}
                  </button>
                ))}
              </div>
              <div className="mt-3 flex justify-between font-mono text-[13px] uppercase text-neutral-600"><span>1 = 10%</span><span>10 = 100%</span></div>
              <button type="button" onClick={() => { setShowProductivityPrompt(false); setProductivityPromptMode('pause'); }} className="mt-4 w-full text-center text-[13px] text-neutral-500 hover:text-white">{productivityPromptMode === 'stop' ? 'Cancel — keep working' : 'Keep timer running'}</button>
            </motion.div>
          </div>
        )}
      ...
      {createPortal(
        <AnimatePresence>
          {showGoals && session && (
            <motion.div className="fixed inset-0 z-[1000] flex items-start justify-center overflow-y-auto bg-black/80 px-4 py-5 backdrop-blur-sm sm:items-center" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setShowGoals(false)}>
              <motion.div initial={{ opacity: 0, scale: .96 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: .96 }} className="max-h-[calc(100dvh-2.5rem)] w-full max-w-xl overflow-y-auto rounded-2xl border border-purple-900/50 bg-neutral-950 p-5 shadow-[0_0_50px_rgba(168,85,247,.12)] sm:p-6" onClick={e => e.stopPropagation()}>
                <div className="flex items-start justify-between gap-4">
                  <div><h2 className="text-lg font-bold text-white">Manage session goals</h2><p className="mt-1 text-xs leading-relaxed text-neutral-400">Add, edit, or review today&apos;s topic goals.</p></div>
                ...
                {(session.goals || []).length > 0 && <div className="mt-5"><div className="text-xs font-bold uppercase tracking-wider text-neutral-400">Current goals · {(session.goals || []).filter(g => goalComplete(g.topicId, g.targetStatus)).length}/{(session.goals || []).length} done</div><div className="mt-2 space-y-2">{(session.goals || []).map(goal => { const topic = topics.find(t => t.id === goal.topicId); if (!topic) return null; const complete = goalComplete(goal.topicId, goal.targetStatus); const priority = priorityDetails(topic.priority); return <div key={goal.id} className={`flex items-center gap-2 rounded-lg border p-2.5 ${editingGoalId === goal.id ? 'border-purple-600 bg-purple-950/15' : complete ? 'border-emerald-900/40 bg-emerald-950/10' : 'border-neutral-800 bg-neutral-900/40'}`}><span className={`flex h-5 w-5 shrink-0 items-center justify-center rounded-full border ${complete ? 'border-emerald-500 bg-emerald-500 text-black' : 'border-neutral-700'}`}>{complete && <Check className="h-3 w-3" />}</span><span className="min-w-0 flex-1"><span className={`block text-[14px] font-semibold ${complete ? 'text-neutral-500 line-through' : 'text-white'}`}>{topic.name}</span><GoalTrail stages={stagesBetween(topic.status, goal.targetStatus)} tone={complete ? 'emerald' : 'cyan'} /><span className="mt-0.5 flex flex-wrap gap-1"><span className={`rounded border px-1.5 py-0.5 text-[12px] font-bold uppercase ${priority.style}`}>P{topic.priority}</span></span></span><span className="flex shrink-0 gap-1"><button onClick={() => { setEditingGoalId(goal.id); setGoalTopicId(goal.topicId); setGoalTarget(goal.targetStatus); }} className="rounded border border-neutral-800 p-1 text-neutral-500 hover:border-purple-700 hover:text-purple-300"><Pencil className="h-3 w-3" /></button><button onClick={() => removeGoal(goal.id)} className="rounded border border-neutral-800 p-1 text-neutral-600 hover:border-rose-800 hover:text-rose-400"><Trash2 className="h-3 w-3" /></button></span></div>; })}</div></div>}

                <div className="mt-5 space-y-5 rounded-2xl border border-neutral-800 bg-neutral-900/30 p-5">
                  <div className="text-xs font-bold uppercase tracking-wider text-neutral-400">{editingGoalId ? 'Edit goal' : 'Add a new goal'}</div>
                  {lastGoalAdded && <div className="text-[13px] text-emerald-300"><Check className="mr-1 inline h-3 w-3" />{lastGoalAdded} added. You can add another.</div>}
                  <div className="block text-xs font-bold uppercase tracking-wider text-neutral-400">Topic{renderTopicPicker()}</div>
                  {selectedTopic && <div className="rounded-lg bg-neutral-950 p-2.5"><div className="truncate text-[14px] font-semibold text-white">{selectedTopic.name}</div><div className="mt-1 text-[13px] uppercase text-cyan-300">Current {stageLabel[selectedTopic.status] || selectedTopic.status} · Priority {selectedTopic.priority}</div></div>}
                  <div className="block text-xs font-bold uppercase tracking-wider text-neutral-400">Milestone to reach today{renderMilestonePicker()}</div>
                  <button disabled={!selectedTopic} onClick={addGoal} className="flex w-full items-center justify-center gap-2 rounded-xl border border-purple-700 bg-purple-950/40 py-3 text-sm font-bold text-purple-200 disabled:opacity-40"><Plus className="h-4 w-4" />{editingGoalId ? 'Save changes' : 'Add goal'}</button>
                  {editingGoalId && <button onClick={() => { setEditingGoalId(null); setGoalTopicId(''); }} className="w-full text-center text-[13px] text-neutral-500 hover:text-white">Cancel editing</button>}
                  {!rankedTopics.length && <div className="text-center text-[13px] text-neutral-600">No unfinished topics available.</div>}
                </div>

                <button onClick={() => setShowGoals(false)} className="mt-5 w-full rounded-xl bg-cyan-500 py-3 text-sm font-bold text-black hover:bg-cyan-400">Done</button>
              </motion.div>
            </motion.div>
          )}
        </AnimatePresence>,
