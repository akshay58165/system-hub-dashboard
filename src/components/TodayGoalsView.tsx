...
export default function TodayGoalsView({ topics, session, setSession }: TodayGoalsViewProps) {
  ...
  const addGoal = () => {
    ...
    setSession({ ...session, goals: [...goals, { id: `goal-${Date.now()}`, topicId: selected.id, topicName: selected.name, targetStatus: target, addedAt: stamp }], updatedAt: stamp });
    setTopicId('');
  };
