...
import { Check, Pause, Play, Plus, RotateCcw, SlidersHorizontal, Target, Trash2 } from 'lucide-react';
...
export default function TodayGoalsView({ topics, session, setSession, onEndSession }: TodayGoalsViewProps) {
  ...
  const pauseResume = () => setSession(current => {
    ...
    if (current.status === 'running' && current.activeSince) return {
      ...current,
      accumulatedActiveMs: current.accumulatedActiveMs + Math.max(0, stamp.getTime() - new Date(current.activeSince).getTime()),
      activeSince: null, pausedAt: stamp.toISOString(), status: 'paused', updatedAt: stamp.toISOString(),
